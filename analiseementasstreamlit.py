...
import openai
import xlsxwriter             # For conditional formatting in Excel
...
# --- 2) Funções com cache para acelerar etapas pesadas ---
@st.cache_data(show_spinner=False)
def parse_ementas(zip_bytes: bytes) -> pd.DataFrame:
    """Extrai e limpa todas as ementas de um ZIP contendo PDFs."""
...
def get_embeddings(texts: list[str], model: SentenceTransformer, batch_size: int = 32) -> np.ndarray:
    """Gera embeddings SBERT em batches e cacheia o resultado."""
...
def name_cluster_with_gpt(prompt: str, api_key: str) -> str:
    """Gera um nome curto para um cluster via GPT-3.5 (cache por prompt)."""
...
if analise == "Clusterização Ementas":
    ...
    for cid in range(k):
        if use_gpt and api_key:
            ...
            # fallback: UC mais próxima do centróide
    ...
    # Download dos clusters em Excel
...
# --- 6B) Matriz de Similaridade ---
    ...
    # Download Excel com formatação rápida (xlsxwriter)
    buf = BytesIO()
    with pd.ExcelWriter(buf, engine='xlsxwriter') as writer:
...
# --- 6C) Matriz de Redundância ---
    ...
    # Download Excel com formatação verde→amarelo→vermelho
    buf = BytesIO()
    with pd.ExcelWriter(buf, engine='xlsxwriter') as writer:
...
# --- 6D) Análise Ementa vs ENADE ---
    ...
    # Coleta resultados por frase ENADE
    records = []
    for i, row in enade_expl.iterrows():
        sims    = simm[i]
